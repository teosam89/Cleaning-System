    ...
    <div class="task-calendar-container">
      ...
      <div class="page-header">
        <div class="header-left">
          ...
          <p class="page-subtitle">Manage and schedule cleaning tasks</p>
        </div>
    ...
    >
      <div class="reschedule-content">
        ...
        <div v-if="rescheduleConfirmation.hasConflict" class="conflict-warning">
          <el-alert
            ...
            show-icon
          >
            <p>Another task is already scheduled for this time. Would you like to:</p>
            <div class="conflict-options">
              <el-button @click="confirmReschedule(false)" type="warning">
                Schedule Anyway
              </el-button>
              <el-button
                v-if="rescheduleConfirmation.conflictResolution"
                @click="confirmReschedule(true)"
                type="success"
              >
                Use Alternative: {{ formatDate(rescheduleConfirmation.conflictResolution?.date) }}
              </el-button>
            </div>
          </el-alert>
        </div>
        ...
        <div v-else class="no-conflict">
          <el-alert
            ...
            show-icon
          >
            <p>No conflicts detected. Confirm to reschedule this task.</p>
          </el-alert>
        </div>
      </div>
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button
            v-if="!rescheduleConfirmation.hasConflict"
            type="primary"
...
export default {
  ...
  setup() {
      ...
      try {
        ...
        if (response.data) {
          calendarStats.value = {
            ...
            upcomingTasks:
              response.data.pendingTasks + response.data.inProgressTasks || 0,
            overdueTasks: response.data.overdueTasks || 0,
          };
        }
      } catch (error) {
        console.warn(
          "Failed to fetch calendar stats, using local data:",
          error
        );
        // Fallback to computed stats from tasks array
        const total = tasks.value.length;
        const completed = tasks.value.filter((t) => t.completed).length;
        const now = new Date();
        const upcoming = tasks.value.filter(
          (t) => !t.completed && new Date(t.startDate) > now
        ).length;
        const overdue = tasks.value.filter(
          (t) => !t.completed && new Date(t.endDate) < now
        ).length;
    ...
    // Fetch assignable users for task assignment
    const fetchAssignableUsers = async () => {
      try {
        const response = await API.get("/api/admin/assignable-users");
        ...
        if (response.data && Array.isArray(response.data)) {
          ...
          tasks.value = response.data.map((task) => {
            ...
            return {
              ...
              // Preserve original fields for drag-and-drop and other operations
      ...
      if (selectedEvent.value) {
        try {
          ...
          if (selectedEvent.value.source === "database") {
            await API.put(`/api/tasks/${selectedEvent.value.id}/complete`);
          }
    ...
    const saveTask = async () => {
      ...
      try {
        ...
        const taskData = {
          ...
          // Send local time as LocalDateTime (no timezone conversion needed)
          scheduledTime: taskForm.startDate
            ? (() => {
                try {
                  ...
                  // Use local time directly for LocalDateTime compatibility
        ...
        if (isEditing.value) {
          ...
          response = await API.put(
            `/api/tasks/${selectedEvent.value.id}`,
            taskData
          );
    ...
    const isValidDropTarget = (targetDate) => {
      ...
      const now = new Date();
      const targetDateTime = new Date(targetDate.date); // Fixed: use 'date' instead of 'fullDate'

      // Only allow dropping on dates after current time (including today)
      const today = new Date();
      today.setHours(0, 0, 0, 0); // Set to start of day

      targetDateTime.setHours(0, 0, 0, 0); // Set to start of day
    ...
    const handleDrop = async (targetDate, event) => {
      console.log("Drop attempted on:", targetDate.date);
      ...
      try {
        ...
        // Preserve original time when moving in month view - only change the date part
        const originalTime = new Date(draggedTask.scheduledTime);
        ...
        rescheduleConfirmation.toDate = newDateTime; // Store date with preserved time
    ...
    const confirmReschedule = async (useAlternative = false) => {
      try {
        ...
        let targetDate = rescheduleConfirmation.toDate; // Already a Date object
        ...
        const taskScheduledTime = new Date(task.scheduledTime);
        ...
        undoStack.value.push({
          action: 'reschedule',
          taskId: task.id,
          originalStartDate: task.scheduledTime,
          originalEndDate: task.dueDate,
          timestamp: new Date()
        });
    ...
    const undoLastAction = async () => {
      ...
      try {
        ...
        ElMessage.error("Failed to undo action");
    ...
    const handleHourDrop = async (date, hour, event) => {
      ...
      try {
        ...
        rescheduleConfirmation.task = draggedTask;
        rescheduleConfirmation.fromDate = formatDateTime(draggedTask.scheduledTime);
        rescheduleConfirmation.toDate = targetDateTime; // Store raw date object instead of formatted string
        rescheduleConfirmation.hasConflict = hasConflict;
    ...
    const handleEditTask = (task) => {
      ...
      selectedEvent.value = {
        ...
        source: "api",
        progressPercentage: task.progressPercentage || 0,
        // Preserve original fields for consistency
        scheduledTime: task.scheduledTime,
        dueDate: task.dueDate,
      };
    ...
    onMounted(async () => {
      ...
      console.log(
        "Task Calendar initialized with real-time MySQL data and auto-refresh"
      );
    });
    ...
    return {
      ...
      // Hour-level drag and drop methods
...
.draggable-task[draggable="true"]:active {
