<template>
  <SupervisorLayout>
      ...
      <div v-else>
        ...
        <div class="announcements-header">
          <div class="header-left">
            <h2 class="page-title">Team Task & Notes</h2>
            <p class="page-subtitle">
              Manage team announcements and weekly routine tasks
            </p>
          </div>
        ...
        <div class="content-grid">
          <!-- Announcements Section (View Only for Supervisor) -->
    ...
    <el-dialog
      ...
      title="📅 Create Weekly Routine Task"
      width="900px"
      :close-on-click-modal="false"
      :z-index="4000"
      append-to-body
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="routineDialogVisible = false">Cancel</el-button>
          <el-button type="primary" @click="submitWeekRoutine">
            Create Routine
          </el-button>
        </div>
      </template>
...
export default {
  ...
  setup() {
    ...
    // Permission checks for supervisor
    const canEditAnnouncement = (announcement) => {
      const currentUserId = AuthUtils.getUserId();
      return announcement.createdById === currentUserId;
    };
    ...
    const canDeleteRoutine = (routine) => {
      const currentUserId = AuthUtils.getUserId();
      return (
        routine.createdByName === "Supervisor" ||
        routine.createdBy === currentUserId
      );
    };
    ...
    const submitAnnouncement = async () => {
      try {
        ...
        const announcementData = {
          ...announcementForm.value,
          createdBy: userInfo.userId || 192, // Fallback to default admin ID
    ...
    const editAnnouncement = (announcement) => {
      ElMessage.info(`Edit announcement: ${announcement.title}`);
    };
    ...
    const submitPublicTask = async () => {
      try {
        ...
        const taskData = {
          ...
          assignedBy: userInfo.userId, // Use actual logged-in user ID
        };
    ...
    const editWeekRoutine = (routine) => {
      ElMessage.info(`Edit routine: ${routine.title}`);
    };
...
/* Header styles - Supervisor Professional Dark Theme */
...
/* Announcements section - Supervisor Dark Theme */
...
/* Task wall section - Supervisor Dark Theme */
.task-wall-section {
...
/* Week routine section - Supervisor Dark Theme */
.routine-section {
...
/* Removed contained-dialog CSS - no longer needed */

/* Input number component improvements */
.full-width-input-number :deep(.el-input-number__decrease),
.full-width-input-number :deep(.el-input-number__increase) {
  z-index: 4005;
}
...
.full-width-input-number :deep(.el-input-number) {
  width: 100%;
  position: relative;
  z-index: 4002;
}

.full-width-input-number :deep(.el-input__wrapper) {
  border-radius: 8px;
  transition: all 0.3s ease;
  position: relative;
  z-index: 4002;
...
.full-width-input-number :deep(.el-input__wrapper:hover) {
  box-shadow: 0 2px 8px rgba(30, 41, 59, 0.15);
}
